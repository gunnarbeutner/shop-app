...
function get_stores() {
	...
	$query = <<<QUERY
SELECT v.`id`, v.`name`, v.`description`, v.`merchant_id`, v.`min_order_count`, v.`min_order_volume`, v.`service_charge_amount`, v.`service_charge_description`, v.`status_message`, v.`rebate_percent`, v.`tracking_id`, m.`name` AS merchant_name, m.`email` AS merchant_email
FROM `stores` v
